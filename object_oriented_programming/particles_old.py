...
class Particle:
    ...
    def __init__(self, volume, p0=1, mass=1, x=None, y=None, z=None, px=None, py=None, pz=None):
        ...
        if px == None or py == None or pz == None:
            ...
            koeff = p0 / np.sqrt(random_values[0]**2 + random_values[1]**2 + random_values[2]**2) # нормировка
            self.px, self.py, self.pz = koeff*random_values
            self.px *= np.random.choice(Particle.SIGNS)
            self.py *= np.random.choice(Particle.SIGNS)
            self.pz *= np.random.choice(Particle.SIGNS)
        else:
            self.px = px
            self.py = py
            self.pz = pz
...
class Center(Particle):
    ...
    def __init__(self, volume, p0=10000, mass=100000, x=None, y=None, z=None, px=10000, py=0, pz=0, is_left=True, sigma=0.5):
        ...
        if px == None or py == None or pz == None:
            self.px = 10000 + 10000*0.15*np.random.random() * np.random.choice(Particle.SIGNS)
            self.py = 10000*0.15*np.random.random() * np.random.choice(Particle.SIGNS)
            self.pz = 10000*0.15*np.random.random() * np.random.choice(Particle.SIGNS)
...
class Volume:
    ...
    def _check_random_dispersion(self, particle: Particle):
        if np.random.rand() <= self._disp_probability:
            self._change_direction(particle)
            
    def _change_direction(self, particle: Particle):
        impulse = np.sqrt(particle.px**2 + particle.py**2 + particle.pz**2)
        changed_particle = Particle(self, 
            p0=impulse, x=particle.x, y=particle.y, z=particle.z)
        particle.px = changed_particle.px
        particle.py = changed_particle.py
        particle.pz = changed_particle.pz

    def _check_x_axis(self, particle: Particle):
    ...
    def _check_y_axis(self, particle: Particle):
    ...
    def _check_z_axis(self, particle: Particle):
    ...
    def _save(self, sec):
        ...
        if sec != 0:
            plt.plot(np.power(10, log_bin_centers), np.power(10, y0 + k*log_bin_centers), 
                label=rf"$k={k:.02f}$")
            plt.legend()
        dir_name = self._make_dir()
        plt.savefig(f"{dir_name}/dist{sec//60}")
        plt.cla()
        plt.clf()
        plt.close()
        
    def _make_dir(self):
        dir_name = "1_simple_volume"
        try:
            os.mkdir(dir_name)
...
class VolumeWithCenters(Volume):
    ...
    def __init__(self, a=1000, b=1000, c=1000, number_of_particles=1000, delta_p=0.2, 
        ...
        for i in range(number_of_centers):
            if i < number_of_centers/(1 + sigma):
                self._centers.append(Center(self, is_left=True))
            else:
                self._centers.append(Center(self, is_left=False))
    ...
    def _check_center(self, center: Center):
        if center.is_left:
            left_x = 0
            right_x = self.a/2
        else:
    ...
    def _create_new(self, center: Center):
        new_center = Center(self, is_left=center.is_left)
    ...
    def _make_dir(self):
        dir_name = "3_volume_with_centers"
        try:
            os.mkdir(dir_name)
        except FileExistsError:
            pass
...
class VolumeWithRandomCenters(VolumeWithCenters):   # same but with Nones in Center()
    ...
    def __init__(self, a=1000, b=1000, c=1000, number_of_particles=1000, delta_p=0.2, 
        ...
        for i in range(number_of_centers):
            if i < number_of_centers/(1 + sigma):
                self._centers.append(Center(self, is_left=True, px=None, py=None, pz=None))
            else:
                self._centers.append(Center(self, is_left=False, px=None, py=None, pz=None))

    def _create_new(self, center: Center):
        new_center = Center(self, is_left=center.is_left, px=None, py=None, pz=None)
        center.x = new_center.x
        center.y = new_center.y
        center.z = new_center.z
        center.px = new_center.px
    ...
    def _make_dir(self):
        dir_name = "4_volume_with_random_centers"
        try:
            os.mkdir(dir_name)
        except FileExistsError:
            pass
...
class LeftOpenedVolumeWithRandomCenters(VolumeWithCenters, LeftOpenedVolume):
    ...
    def _make_dir(self):
        dir_name = "5_left_opened_volume_with_random_centers"
        try:
            os.mkdir(dir_name)
        except FileExistsError:
            pass
...
class UltimateVolumeWithEnergyLoss(LeftOpenedVolumeWithRandomCenters):
    ...
    def _check_energy_loss(self, particle: Particle):
    ...
    def _make_dir(self):
        dir_name = "6_volume_with_energy loss"
        try:
            os.mkdir(dir_name)
        except FileExistsError:
            pass
